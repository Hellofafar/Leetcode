...
class Solution:
    def canCross(self, stones: List[int]) -> bool:
        # mappting: postion -> set of jumpsize
        # value is a set containing jumpsize which can lead to the current stone position
        jumpMap = {x:set() for x in stones}
        jumpMap[0].add(0)
        
        for pos in stones:
            if jumpMap[pos]:
                # if set of jumpsize is not null
                for k in jumpMap[pos]:
                    # if can reach next stone by jumpping k - 1, k or k + 1 steps, append the jumpsize to value of corresponding key in jumpMap
                    for step in range(k-1, k+2):
                        if step > 0 and pos + step in jumpMap:
                            jumpMap[pos+step].add(step)
...
# ------------------------------
# Summary:
# DP solution from https://leetcode.com/problems/frog-jump/solution/
# 
# O(N^2) time O(N^2) space
# But someone doubted it should be O(N * sqrt(N)): https://leetcode.com/problems/frog-jump/solution/