...
class Solution:
    def validUtf8(self, data: List[int]) -> bool:
        bytesCount = 0  # Expected number of bytes
        for num in data:
            binstr = bin(num)[2:].zfill(8)
            if bytesCount == 0:
                # Get the starting 1's count
                for i in range(8):
                    if binstr[i] == "1":
                        bytesCount += 1
                    else:
                ...
                if bytesCount == 0:
                    continue
                elif bytesCount == 1 or bytesCount > 4:
                    return False
...
# ------------------------------
# Summary:
# Get an expected number of bytes count at first, then check if the number is exactly matched.
# Note the expected number can only be [0, 2, 3, 4]. 1 or over 5 will return false.