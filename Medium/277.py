...
# Note:
# 
# The directed graph is represented as an adjacency matrix, which is an n x n matrix where 
...
class Solution(object):
    def findCelebrity(self, n):
        ...
        :rtype: int
        """
        # first round to find a candidate who doesn't konw any of other people
        # some one who knows anyther people is not a celebraty candidate
        candidate = 0
        for i in range(1, n):
            if knows(candidate, i):
                candidate = i
        # verify the candidate
        # if the candidate knows some one else or some one else doesn't know him
        # the candidate is not a celebratt
        for i in range(n):
...
# ------------------------------
# Summary:
# Two pass solution from: https://leetcode.com/problems/find-the-celebrity/discuss/71227/Java-Solution.-Two-Pass
