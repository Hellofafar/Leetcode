...
class Solution:
    def findCircleNum(self, M: List[List[int]]) -> int:
        N = len(M)
        student = [1] * N # 1 represent not visited, 0 represents visited
        
        res = 0
        for i in range(N):
            if student[i] == 1:
                # this student is not checked for circle yet
                circle = collections.deque([i])
                while circle:
                    # check all relative friends in the circle
                    stu = circle.popleft()
                    student[stu] = 0
                    for j in range(N):
                        # look for j's direct friends who haven't been detected
                        if student[j] == 1 and M[stu][j] == 1:
                            circle.append(j)                    
...
# ------------------------------
# Summary:
# BFS solution. Use student list as a visited record
# This problem can also be solved with DFS solution, e.g: https://leetcode.com/problems/friend-circles/discuss/101338/Neat-DFS-java-solution